...
type VoiceState = 'idle' | 'listening' | 'processing' | 'clarifying' | 'confirming';
...
export function VoiceTaskModal({ user, onClose, onSaved }: { user: User, onClose: () => void, onSaved: () => void }) {
  ...
  const handleProcess = async (text: string) => {
    ...
    // If clarifying, append new info to old parsed data
    let prompt = text;
    if (parsedData && voiceState === 'clarifying') {
      prompt = `Context from earlier: Task: ${parsedData.task}, Date: ${parsedData.date}, Time: ${parsedData.time}. User now says: "${text}". Please combine and fill in the missing pieces.`;
    }
